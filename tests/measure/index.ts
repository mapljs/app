...
export const measureAppAOT = async (app: AnyRouter) => {
  await Bun.write(`${import.meta.dir}/fetch.js`, `export default ${aotfn(app, { excludeStatic: true })};`);
  // @ts-ignore
...
export const measureAppJIT = async (app: AnyRouter) =>
  console.log(`JIT compilation: ${await measureAvg(async () => await jitc(app, { excludeStatic: true }))}`);
